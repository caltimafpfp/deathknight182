...
function renderGuide(id){const section=guideData.find(item=>item.id===id);if(!section)return;const saved=siteGuides[id]||{};const heading=saved.title||siteButtons[guideKeys[id]]?.label||section.title;const intro=saved.intro||section.intro;const content=saved.content||'';const paragraphs=content.split(/\n{2,}/).filter(Boolean).map(text=>`<p>${safeText(text).replaceAll('\n','<br>')}</p>`).join('');const gallery=(saved.images||[]).map((src,index)=>`<img src="${safeText(src)}" alt="${safeText(heading)}圖片 ${index+1}" loading="lazy">`).join('');tabs.querySelectorAll('.guide-tab').forEach(btn=>btn.classList.toggle('active',btn.dataset.id===id));detail.innerHTML=`<header><div><h3>${safeText(heading)}</h3><p>${safeText(intro)}</p></div><a class="more-link" href="#game-guide">返回分類</a></header><div class="guide-content-body">${paragraphs}</div>${gallery?`<div class="guide-content-gallery">${gallery}</div>`:''}`;detail.classList.add('open');detail.scrollIntoView({behavior:'smooth',block:'center'})}
